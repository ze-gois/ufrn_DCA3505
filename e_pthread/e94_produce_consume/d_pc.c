...
int main (){
    struct THREAD_ARG prod_args[N_PRODUCERS];
    for (int p = 0; p < N_PRODUCERS; p++){
        prod_args[p].id = p;
        pthread_create(&prod_args[p].tid, NULL, produtor, (void *) &prod_args[p]);
    }

    struct THREAD_ARG cons_args[N_CONSUMERS];
    for (int c = 0; c < N_CONSUMERS; c++){
        cons_args[c].id = c;
        pthread_create(&(cons_args[c].tid), NULL, consumidor, (void *) &prod_args[c]);
    }
