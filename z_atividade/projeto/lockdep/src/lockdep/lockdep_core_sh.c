...
void graph_visualize() {
    ...
    Vector2* positions = malloc(node_count * sizeof(Vector2));
    if (positions == NULL) {
        CloseWindow();
        return;
    }
    ...
    for (size_t i = 0; i < node_count; i++) {
        float angle = (2.0f * PI * (float)i) / (float)node_count;
    ...
    while (!WindowShouldClose()) {
        ...
        DrawArrow(positions[0],positions[1],BLACK);
        // lock_node_t *lock = lock_registry;

        // while(lock){
        //     lock_node_t* ls = lock_registry;
        //     size_t source_index = 0;
        //     while(ls && ls != lock) {
        //         source_index++;
        //         ls = ls->next;
        //     }
        //     adjacency_locks_t* to_visit = lock->children;

        //     Vector2 start = positions[source_index];

        //     while(to_visit){
        //         lock_node_t* ls = lock_registry;
        //         size_t sink_index = 0;
        //         while(ls && ls != to_visit->lock) {
        //             sink_index++;
        //             ls = ls->next;
        //         }
        //         Vector2 end = positions[sink_index];
        ...
        //         ls = ls->next;
        //     }
        //     Vector2 pos = positions[source_index];
        //     DrawCircleV(pos, 20.0f, BLUE);

        //     const char* label;
        //     label = TextFormat("%zu", source_index);
